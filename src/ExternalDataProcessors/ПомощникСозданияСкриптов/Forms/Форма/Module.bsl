...
Процедура ТаблицаПараметровПредсталвлениеПараметраПриИзменении(Элемент)
	...
	Если СтрНачинаетсяС(ТекущиеДанные.ПредсталвлениеПараметра, "<") И СтрЗаканчиваетсяНа(ТекущиеДанные.ПредсталвлениеПараметра, ">") Тогда
		ТекущиеДанные.Параметр = "";
		ТекущиеДанные.ПредставлениеПустогоЗначения = Сред(ТекущиеДанные.ПредсталвлениеПараметра, 1, СтрДлина(ТекущиеДанные.ПредсталвлениеПараметра) - 2);
	Иначе
...
Процедура РазобратьСтрокуСкрипта()
	...
	МассивИменПрограмм = Элементы.Программа.СписокВыбора.ВыгрузитьЗначения();
	
	ПодстрокаИменПрограмм = СтрСоединить(МассивИменПрограмм, "|");
	ШаблоныРазбораСкрипта	= ШаблоныРазбораСкриптаПоПрограммам();
	
	ПараметрыШаблоновСкрипта = Новый Структура;
	ПараметрыШаблоновСкрипта.Вставить("ПодстрокаИменПрограмм",	ПодстрокаИменПрограмм);
	ПараметрыШаблоновСкрипта.Вставить("ШаблоныРазбораСкрипта",	ШаблоныРазбораСкрипта);
	
	СтруктураСкрипта = СтруктураСкрипта(ТекстСкриптаПредставление, ПараметрыШаблоновСкрипта);
	
	УстановитьАктивнойстьКоманд(ДеревоКоманд, СтруктураСкрипта.СтруктураКомманд);
	
	
//	Для Каждого ЭлементСкрипта Из СтруктураСкрипта.СтруктураКомманд Цикл
//		
//		
//		
//		УстановитьАктивнойстьКоманд(ДеревоКоманд, СтруктураСкрипта.СтруктураКомманд);
//		
//		Если СтрокаДереваКоманд = Неопределено Тогда
//			СтрокиПараметров = ТаблицаПараметров.НайтиСтроки(Новый Структура("Параметр", ЭлементСкрипта.Ключ));
//		Иначе
//			СтрокиПараметров = ТаблицаПараметров.НайтиСтроки(Новый Структура("КлючСвязи", СтрокаДереваКоманд));
//		КонецЕсли;
//			
//		Если СтрокиПараметров.Количество() > 0 Тогда
//			ЗаполнитьПараметры(СтрокиПараметров, ЭлементСкрипта.Значение);
//		КонецЕсли;
//		
//	КонецЦикла;
...
&НаКлиенте
Процедура УстановитьАктивнойстьКоманд(ЭлементДерева, СтруктураКомманд)
	
	Если Не ЗначениеЗаполнено(СтруктураКомманд) Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементыДереваКоманд = ЭлементДерева.ПолучитьЭлементы();
	
	Для Каждого СтрокаКоманды Из ЭлементыДереваКоманд Цикл
		
		КомандаСкрипта = СтруктураКомманд.Получить(СтрокаКоманды.Команда);
		Если КомандаСкрипта <> Неопределено Тогда
			
			СтрокаКоманды.Использование = Истина;
			
			СтрокиПараметров = ТаблицаПараметров.НайтиСтроки(Новый Структура("КлючСвязи", СтрокаКоманды.КлючСвязи));
			
			Для Каждого СтрокаПараметра Из СтрокиПараметров Цикл
				
				Если ЗначениеЗаполнено(КомандаСкрипта) Тогда
					ЗначенияКомандыСкрипта = КомандаСкрипта.Получить(СтрокаПараметра.Параметр);
				Иначе
					ЗначенияКомандыСкрипта = Неопределено;
				КонецЕсли;
				
				Если ЗначенияКомандыСкрипта = Неопределено Тогда
					
					ЗначенияКомандыСкрипта = СтруктураКомманд.Получить(СтрокаПараметра.Параметр);
					Если ЗначенияКомандыСкрипта <> Неопределено Тогда
						
						СтрокаПараметра.Использование = Истина;
						
						Если ЗначенияКомандыСкрипта.Количество() = 1 Тогда
							СтрокаПараметра.ЗначениеАргумента = ЗначенияКомандыСкрипта[0].Значение;
						КонецЕсли;
						
						СтруктураКомманд.Удалить(СтрокаПараметра.Параметр);
				...
				Иначе
					СтрокаПараметра.Использование = Истина;
					СтрокаПараметра.ЗначениеАргумента = КомандаСкрипта.Значение;
				КонецЕсли;
				
			КонецЦикла;
			
			СтруктураКомманд.Удалить(СтрокаКоманды.Команда);
			
		КонецЕсли;
		
		УстановитьАктивнойстьКоманд(СтрокаКоманды, СтруктураКомманд);
...
Функция КлючДереваКоманд(ИмяКоманды, ЭлементДерева)
	...
	Для Каждого СтрокаКоманды Из ЭлементыДереваКоманд Цикл
		...
		Если СтрокаКоманды.Команда = ИмяКоманды Тогда
			Возврат СтрокаКоманды.КлючСвязи;
		Иначе
			КлючСвязи = КлючДереваКоманд(ИмяКоманды, СтрокаКоманды);
			Если КлючСвязи <> Неопределено Тогда
				Возврат КлючСвязи;
			КонецЕсли;
...
Функция СтруктураСкрипта(ТекстСкрипта, ПараметрыШаблоновСкрипта);
	...
	СтруктураСкрипта = Новый Структура("ПутьКПрограмме, Программа, СтруктураКомманд");
		...
		Если ГруппыПутиИИмениПрограммы.Количество() > 1 Тогда
			...
			Для Каждого КомандаСкрипта Из ГруппыКомандСкрипта Цикл
				...
				Если ГруппаКоманды.Количество() > 1 Тогда
					...
					Для Каждого ПараметрКоманды Из ГруппыПараметорвКоманды Цикл
						...
						Если ГруппаПараметровИЗначений.Количество() > 1 Тогда
							...
							ИначеЕсли Не ЗначениеЗаполнено(ИмяПараметра) Тогда
								ПараметрИЗначение.Вставить(ИмяКоманды, ЗначениеАргумента);
							Иначе
					...
					КомандыИПараметры.Вставить(ИмяКоманды, ПараметрИЗначение);
	...
	СтруктураСкрипта.СтруктураКомманд = КомандыИПараметры;
...
Функция ШаблоныРазбораСкриптаПоПрограммам()
	...
	// Заполнение шаблонов для 1cedtcli
	СтруктураШаблона = Новый Структура;
	СтруктураШаблона.Вставить("ШаблонПоискаКоманд",		"([\/][\w]+)([^\/.]*|[+-])");
	СтруктураШаблона.Вставить("ШаблонПоискаПараметров",	"(-\w+)?([^-]*|[+-])");
	...
	// Заполнение шаблонов для webinst
	СтруктураШаблона = Новый Структура;
	СтруктураШаблона.Вставить("ШаблонПоискаКоманд",		"([\/][\w]+)([^\/.]*|[+-])");
	СтруктураШаблона.Вставить("ШаблонПоискаПараметров",	"(-\w+)?([^-]*|[+-])");
	...
	Возврат ШаблоныПоискаПоПрограммам;
