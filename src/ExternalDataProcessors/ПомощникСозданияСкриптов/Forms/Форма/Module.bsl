...
Процедура УстановитьАргументКоманды(СтрокаТаблицыАргументов, СтрокаАргумента, АргументКоманды)
	...
	Если РазделительПробел = 0 Или РазделительКовычка = 0 Тогда
		СтрокаПарметра[0].Аргумент = СокрЛП(СтрокаАргумента);	
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		РазделительКовычка = СтрНайти(СтрокаАргумента, """", НаправлениеПоиска.СКонца);				
		
		СтрокаАргументаКоманды = СокрЛП(Лев(СтрокаАргумента, РазделительКовычка + 1));
