...
&НаКлиенте
Процедура ПолучитьНаборПараметров(СтрокаКомманд)
	
	МассивКоманд = СтрРазделить(СтрокаКомманд, " ");
	
	// Проверить!!!!
	
	Для Каждого ИндексМассива Из МассивКоманд.ВГраница() Цикл
		
		МассивПоказателя = СтрРазделить(МассивКоманд[ИндексМассива], "=");
		
		Если МассивПоказателя.Количество() > 0 Тогда
			МассивКоманд[ИндексМассива] = МассивПоказателя[0] + "=";
			МассивКоманд.Вставить(ИндексМассива + 1, МассивПоказателя[1]);		
		КонецЕсли;	
...
Процедура УстановитьАргументПараметра(КлючСвязи, Параметр, СтрокаАргумента)
	...
	Если СтрокаПарметра.Количество() > 0 Тогда
		СтрокаПарметра[0].Аргумент = СтрокаАргумента;
