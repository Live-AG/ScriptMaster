...
Асинх Процедура ЗагрузитьДанные(Команда)
	...
	ПараметрыДиалога.Фильтр = "Файл JSON|*.json|Файл XML|*.xml";
	...
	ОписаниеФайла = Ждать ПоместитьФайлНаСерверАсинх(ПрогрессОбратныйВызов, ПередНачаломОбратныйВызов, , ПараметрыДиалога);
	
	Сообщить("Помещение файла " + ?(ОписаниеФайла.ПомещениеФайлаОтменено, "отменено", "выполнено успешно"));
	Сообщить("Адрес во временном хранилище: " + ОписаниеФайла.Адрес);
	Сообщить("Имя файла: " + ОписаниеФайла.СсылкаНаФайл.Имя);
	Сообщить("Размер файла, байт: " + ОписаниеФайла.СсылкаНаФайл.Размер());

//	ДиалогВыбораФайлов = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
//	ДиалогВыбораФайлов.Заголовок = "Выбор файла для сохранения";
//	ДиалогВыбораФайлов.МножественныйВыбор = Ложь;
//	//ДиалогВыбораФайлов.Фильтр = "Файл XML|*.xml";
//	ДиалогВыбораФайлов.Фильтр = "Файл JSON|*.json|Файл XML|*.xml";
//
//	Обещание = ДиалогВыбораФайлов.ВыбратьАсинх();
//	ВыбранныйФайл = Ждать Обещание;

	Если ОписаниеФайла <> Неопределено Тогда
		ЗагрузитьНастройкуНаСервере(ОписаниеФайла.Адрес, ОписаниеФайла.СсылкаНаФайл.Расширение);
...
Процедура ПрогрессОбратныйВызов(ПомещаемыйФайл, Помещено, ОтказОтПомещенияФайла, ДополнительныеПараметры) Экспорт
   // Состояние("Файл " + ПомещаемыйФайл.Имя, Помещено, "Размер файла, байт = " + ПомещаемыйФайл.Размер(), БиблиотекаКартинок.Документ);
КонецПроцедуры
&НаКлиенте
Процедура ПередНачаломОбратныйВызов(ПомещаемыйФайл, ОтказОтПомещенияФайла, ДополнительныеПараметры) Экспорт
   // Сообщить("Помещаемый файл - " + ПомещаемыйФайл.Имя + ", размер файла, байт = " + ПомещаемыйФайл.Размер());
КонецПроцедуры
...
Процедура ПрочитатьПараметрыИзМакета()
	...
	Попытка
		...
		ЗагрузитьНастройкуНаСервере(СтрокаДанных, "xml");	
	Исключение
