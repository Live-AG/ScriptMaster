...
Процедура ЗаполнитьАгрументыПараметров(СтрокаКомманд, НаборЭлементовСкрипта)
	...
	//ТаблицаАргументов.Колонки.Добавить("Аргумент");
	...
	Для Каждого СтрокаТаблицыАргументов Из ТаблицаАргументов Цикл
		...
		Аргумент = Сред(СтрокаКомманд, НачальнаяПозицияАргумента, ДлинаСтрокиАргумента);
		...
		Если СтрокаПарметра.Количество() > 0 Тогда
			СтрокаПарметра[0].Аргумент = СокрЛП(Аргумент);	
		КонецЕсли;
	КонецЦикла;
