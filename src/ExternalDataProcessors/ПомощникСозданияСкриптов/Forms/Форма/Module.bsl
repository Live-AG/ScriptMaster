...
Процедура РазобратьСтрокуСкрипта()
	...
	СтрокаКомманд = Прав(ТекстСкриптаПредставление, ДлинаСтрокиСкрипта - ПозицияИмениПрограммы - СтрДлина(Программа));
...
Процедура ЗаполнитьАгрументыПараметров(СтрокаКомманд, НаборЭлементовСкрипта)
	...
	Для Каждого СтрокаТаблицыАргументов Из ТаблицаАргументов Цикл
		Аргумент = Сред(СтрокаКомманд, 
						СтрокаТаблицыАргументов.ПозицияНачала, 
						СтрДлина(СтрокаТаблицыАргументов.ПараметрБезАргумента));
		...
		Если СтрокаПарметра.Количество() > 0 Тогда
			СтрокаПарметра[0].Аргумент = Аргумент;	
		КонецЕсли;
	КонецЦикла;
...
Процедура ОпределитьКомандыСкрипта(СтрокаКомманд, ЭлементыДерева, НаборЭлементовСкрипта)
	...
	Для Каждого ЭлементДерева Из СтрокиДерева Цикл
		...
		Если ПозицияНайденнойКоманды >= ПозицияКурсораПоиска Тогда
...
Процедура ОпределитьПараметрыСкрипта(КлючСвязи, СтрокаКомманд, ПозицияКурсораПоиска, НаборЭлементовСкрипта)
	...
	Для Каждого СтрокаПараметра Из НайденныеСтрокиПараметров Цикл
		...
		Если ПозицияНайденногоПараметра > 0 Тогда
			СтрокаПараметра.Использование = Истина;
			ЗаписатьВНаборЭлементовСкрипта(НаборЭлементовСкрипта, ПозицияКурсораПоиска, СтрокаПараметра);
...
Процедура ЗаписатьВНаборЭлементовСкрипта(НаборЭлементовСкрипта, ПозицияКурсораПоиска, ДанныеСтроки = Неопределено)
	...
	СтруктураЭлемента.Вставить("КлючСтроки");
	...
	Если ДанныеСтроки <> Неопределено Тогда
		...
		СтруктураЭлемента.КлючСтроки = ДанныеСтроки.КлючСвязи;
	КонецЕсли;
