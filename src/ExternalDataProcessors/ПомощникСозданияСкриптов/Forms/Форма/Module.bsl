...
Процедура РазобратьСтрокуСкрипта()
	...
	ДанныеСтроки.Вставить("КлючСвязи", "");
	ДанныеСтроки.Вставить("Параметр", "");
	ДанныеСтроки.Вставить("ИмяАргумента", "");
	
	ЗаписатьВНаборЭлементовСкрипта(НаборЭлементовСкрипта, 1, ДанныеСтроки);
...
// Заполнить агрументы параметров.
// 
// Параметры:
//  СтрокаКомманд Строка комманд
//  НаборЭлементовСкрипта Набор элементов скрипта
&НаСервере
Процедура ЗаполнитьАгрументыПараметров(СтрокаКомманд, НаборЭлементовСкрипта)
	...
	Для Каждого СтрокаТаблицыАргументов Из ТаблицаАргументов Цикл
		...
		АргументКоманды = АргументыКоманд.Получить(СтрокаТаблицыАргументов.КлючСвязи);
		Если ЗначениеЗаполнено(АргументКоманды) Тогда
			УстановитьАргументКоманды(СтрокаТаблицыАргументов, СтрокаАргумента, АргументКоманды);	
		КонецЕсли;
...
&НаСервере
Процедура УстановитьАргументКоманды(СтрокаТаблицыАргументов, СтрокаАргумента, АргументКоманды)
	
	// "C:/Program Files/Temp" -force
	// -force "C:/Program Files/Temp" 
	// -force
	// "C:/Program Files/Temp"
	// -force -out
	
	// Не реализованный вариант: оба параметра в ковычках и 
	// "Мое ""любимое"" расширение!" "C:/Program Files/Temp/myext.cfu"
	
	РазделительПробел	= СтрНайти(СтрокаАргумента, " ");
	РазделительКовычка	= СтрНайти(СтрокаАргумента, """");
	
	Если РазделительПробел = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьВНаборЭлементовСкрипта(НаборЭлементовСкрипта, ПозицияКурсораПоиска, ДанныеСтроки)
	...
	Если ДанныеСтроки <> Неопределено Тогда
		...
		Иначе	
			СтруктураЭлемента.ПараметрБезАргумента = СокрЛП(СтрЗаменить(ДанныеСтроки.Параметр, "<" + ДанныеСтроки.ИмяАргумента + ">", ""));
		КонецЕсли;
		
		Если СокрЛП(ДанныеСтроки.Параметр) = "<" + СокрЛП(ДанныеСтроки.ИмяАргумента) + ">" Тогда
...
&НаКлиенте
Процедура ОтметитьНаличиеАргументаБезПараметра(НаборЭлементовСкрипта, ИмяПараметра, КлючСвязи, ПозицияКурсораПоиска)	
