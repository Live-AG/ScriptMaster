...
Асинх Процедура ПрограммаПриИзменении(Элемент)
	...
	ПриИзмененииПрограммы();

КонецПроцедуры

&НаКлиенте
Процедура ПриИзмененииПрограммы()
...
Процедура ТаблицаПараметровПараметрПриИзменении(Элемент)
	...
	ТекущиеДанные = Элементы.ТаблицаПараметров.ТекущиеДанные;
//	Разделитель = ?(ТекущиеДанные.Параметр = "", "", РазделительПараметраИАргумента);
//	ПредставлениеЗначения = ?(ТекущиеДанные.ПредставлениеПустогоЗначения = "", 
//									"", 
...
Функция ПолучитьТекстИПердставлениеСкрипта()
	...
	ЗаполнитьПараметрыКоманды(МассивФорматированныхСтроки, СтрокаТекстаСкрипта, СтрокиДереваКоманд);
	
	ФорматированныйТекстСкрипта = Новый ФорматированнаяСтрока(МассивФорматированныхСтроки);
	СтрокаТекстаСкрипта = СтрокаПутиКФайлу + СокрЛП(СтрокаТекстаСкрипта);
	СтрокаТекстаСкрипта = УбратьЛишниеСимволы(СтрокаТекстаСкрипта);
	
	СтруктураТекстаСкрипта = Новый Структура("ТекстСкрипта, ФорматированныйТекстСкрипта");
	СтруктураТекстаСкрипта.ТекстСкрипта = СтрокаТекстаСкрипта;
...
Процедура ЗаполнитьСтрокуПараметра(МассивФорматированныхСтроки, ТекстСкриптаПредставление, СтруктураСтроки)
	...
	СтрокаПараметра = СтруктураСтроки.Параметр + РазделительПараметраИАргумента + СтруктураСтроки.ЗначениеАргумента;
		
	МассивФорматированныхСтроки.Добавить(Новый ФорматированнаяСтрока(" " + СокрЛП(СтрокаПараметра), , ЦветПараметра));
	...
	МассивФорматированныхСтроки.Добавить(Новый ФорматированнаяСтрока(РазделительПараметраИАргумента + СокрЛП(ПредставлениеАргумента), , ЦветАргумента));
	ТекстСкриптаПредставление = ТекстСкриптаПредставление + РазделительПараметраИАргумента + СокрЛП(ПредставлениеАргумента);
...
//		КонецЕсли;
//		
//		Если ПустаяСтрока(СтрокаПараметров.ПредставлениеПустогоЗначения) Или ПустаяСтрока(СтрокаПараметров.ТипАргумента) Тогда 
//			ПредставлениеЗначенияАргумента = "";
//		Иначе
//			ПредставлениеЗначенияАргумента = ?(ПустаяСтрока(СтрокаПараметров.Параметр), "", РазделительПараметраИАргумента)
...
Процедура РазобратьСтрокуСкрипта()
	...
	Программа = СтруктураСкрипта.Программа;
	ПриИзмененииПрограммы();
